...
Context
The current product is a single-tenant voice kiosk hardcoded for Savour Foods: Gemini ←→ server.ts ←→ BACKEND_URL (Render) ←→ Supabase
...
Restaurant info (name, logo, timezone)
Select adapter type with explanation of each
Configure adapter (paste API URL + key for CustomApiAdapter; or auto for Managed)
Test connection (we call their API, show pass/fail)
Customize Gemini persona (name, voice, extra prompt, languages)
Set business rules → generate kiosk URL: app.yourdomain.com/kiosk/{slug}
